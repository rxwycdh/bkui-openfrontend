...
axiosInstance.interceptors.request.use(config => {
    ...
    // 发起请求前添加token header
    console.log('对url', config.url, '注入header')
    injectTokenToHeaders()
    return config
...
async function getPromise (method, url, data, userConfig = {}) {
    ...
    promise = new Promise(async (resolve, reject) => {
        ...
        // axiosInstance.defaults.headers.common['aaa'] = 'bbb'
        //
        // console.log('看看这里面的头', axiosInstance.defaults.headers)
        try {
...
export function injectTokenToHeaders () {
    const token = store.state.token
    console.log('[inject] token, ', token)
    console.log('[inject] instance, ', axiosInstance)

    if (token === '' || token === undefined) {
        console.warn('Can not find token, redirect to login...')
    }
